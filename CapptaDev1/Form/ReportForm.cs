...
namespace SinalVeiculos
{
    public partial class ReportForm : Form
    {
        ...
        private void BtnSearchCustomerForName_Click(object sender, EventArgs e)
        {
            ...
            {
                var customerRegister = new CustomerRegiser();
                this.dgvCustomer.DataSource = customerRegister.GetAll().Where(customer => customer.Name == txtSearchCarForName.Text).ToList();
            }
            else
            {
        ...
        private void BtnSearcheCarForName_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtSearchCarForName.Text))
            {
                var carRegister = new CarRegister();
                this.dgvCar.DataSource = carRegister.GetAll().Where(car => car.Name == this.txtSearchCarForName.Text).ToList();
            }
            else
            {
        ...
        private void BtnSearchRequestForDate_Click(object sender, EventArgs e)
        {
            var requestRegister = new RequestRegister();
